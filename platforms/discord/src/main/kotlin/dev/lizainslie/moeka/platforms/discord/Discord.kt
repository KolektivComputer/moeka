package dev.lizainslie.moeka.platforms.discord
...
object Discord : PlatformAdapter<DiscordCommandConfig>(
                ...
                is String -> {
                    val channelId =
                        if (value.startsWith("<#") && value.endsWith(">")) {
                            // this is a formatted mention
                            value.substring(2, value.length - 1)
                        } else if (value.all { it.isDigit() }) {
                            // this is a raw ID
                            value
                        } else {
                            throw IllegalArgumentException("Invalid channel ID format: $value") // ohno.jpg
                        }
                ...
                is String -> {
                    val roleId =
                        if (value.startsWith("<@&") && value.endsWith(">")) {
                            ...
                            throw IllegalArgumentException("Invalid role ID format: $value") // ohno.jpg
                        }
                    ...
                    throw IllegalArgumentException("Invalid role type: ${value::class.simpleName}")
                }
            }
        }
    ...
    fun parseEmojiIdentifier(value: String): String =
        if (value.startsWith("<:") && value.endsWith(">")) {
            ...
            value // assume it's a unicode emoji and return as is
        }
    ...
    override suspend fun registerCommand(
        ...
        if (!command.communityOnly && module.visibility != PluginVisibility.DEVELOPER && config.registerCommandsGlobally) {
        ...
        for (guildConf in config.guilds) {
            if (module.visibility == PluginVisibility.DEVELOPER && !guildConf.admin) continue
    ...
    override suspend fun unregisterCommand(
        ...
        if (!command.communityOnly && config.registerCommandsGlobally) {
            ...
            val commands =
                kord.rest.interaction
                    .getGlobalApplicationCommands(kord.selfId)
                    .toList()
        ...
        for (guildConf in config.guilds) {
            ...
            val commands =
                kord.rest.interaction
                    .getGuildApplicationCommands(kord.selfId, guildConf.id)
                    .toList()
    ...
    suspend fun getGuildById(id: Snowflake) =
        try {
            kord.getGuild(id)
    ...
    private suspend fun GuildChatInputCommandInteractionCreateEvent.handle() {
        this@Discord.suspendLogPlatform {
            ...
            suspendLogPlugin(registration.module) {
