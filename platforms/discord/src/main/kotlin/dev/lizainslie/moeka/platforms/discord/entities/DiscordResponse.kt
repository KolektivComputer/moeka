...
interface DiscordResponse : PlatformResponse {
    ...
    suspend fun createPrivateFollowup(
        text: String = "",
        block: EmbedBuilder.() -> Unit,
    ): DiscordResponse

    suspend fun createStealthFollowup(
        content: String = "",
        block: EmbedBuilder.() -> Unit,
    ) = if (context.callerIsStealth()) {
        createPrivateFollowup(content, block)
    } else {
        createFollowup(content, block)
    }
}
