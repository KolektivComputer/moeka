...
class DiscordSlashCommandContext(
    ...
    override suspend fun respondPrivate(text: String): DiscordSlashCommandResponse =
        (
            response?.createPrivateFollowup(text) ?: let {
                val kordResponse =
    ...
    ): DiscordSlashCommandResponse =
        (
            (response as DiscordSlashCommandResponse?)?.createPrivateFollowup(text, block) ?: let {
                val kordResponse =
                    interaction.respondPublic {
                        content = text
                        embed(block)
    ...
    override suspend fun respondPrivate(
        text: String,
        block: EmbedBuilder.() -> Unit,
    ): DiscordSlashCommandResponse =
        (
